...
def download_Video_Audio(path, vid_url, file_no):
    ...
    try:  # Tries to find the video in 720p
        ...
        video = sorted(yt.filter("mp4"), key=lambda video: int(
            video.resolution[:-1]), reverse=True)[0]

        print("downloading", yt.title+" Video and Audio...")
    try:
        bar = progressBar()
        print(video)
        video.download(path, filename=yt.title+'.mp3')
    ...
    try:
        ...
        aud = 'ffmpeg -i '+str(file_no)+'.mp3'+' '+str(file_no)+'.wav'
